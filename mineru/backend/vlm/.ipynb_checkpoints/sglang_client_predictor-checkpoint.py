...
class SglangClientPredictor(BasePredictor):
    ...
    def batch_predict_better(
        self,
        image_gen: Union[
            Iterable[dict], AsyncIterable[dict]
        ],  # 可以是生成器或异步生成器
        ...
        no_repeat_ngram_size: Optional[int] = None,
        max_new_tokens: Optional[int] = None,
        max_concurrency: int = 100,  # 本来为100
        ...
        task = self.batch_predict_pdf_async(
            image_gen=image_gen,
            prompts=prompts,
            temperature=temperature,
            top_p=top_p,
    ...
    async def batch_predict_pdf_async(
        self,
        image_gen: Union[Iterable[dict], AsyncIterable[dict]],
        prompts: Union[Iterable[str], str] = "",
        temperature: Optional[float] = None,
        top_p: Optional[float] = None,
        top_k: Optional[int] = None,
        ...
        no_repeat_ngram_size: Optional[int] = None,
        max_new_tokens: Optional[int] = None,
        max_concurrency: int = 10,  # dy 本来是100
    ) -> Tuple[List[str], List[dict]]:
        """
        image_gen: 同步或异步生成器，每个元素为 image_dict
        返回 (推理结果列表, 完整images列表)
        """
        ...
        async def producer():
            idx = 0
            if hasattr(image_gen, "__aiter__"):
                async for image_dict in image_gen:
                    await queue.put((idx, image_dict))
                    images_list.append(image_dict)
                    idx += 1
            else:
                for image_dict in image_gen:
                    await queue.put((idx, image_dict))
                    images_list.append(image_dict)
                    idx += 1
            # 结束信号
            for _ in range(max_concurrency):
                await queue.put((None, stop_token))

        # 消费者
        async def consumer():
            if isinstance(prompts, str):
                prompts_iter = (prompts for _ in range(1 << 60))
            else:
                prompts_iter = iter(prompts)
            while True:
                idx, image_dict = await queue.get()
                if image_dict is stop_token:
                    break
                try:
        ...
        consumer_tasks = [
            asyncio.create_task(consumer()) for _ in range(max_concurrency)
        ]
        await asyncio.gather(producer_task, *consumer_tasks)
        # 按输入顺序排序
        results.sort(key=lambda x: x[0])
        return [r[1] for r in results], images_list
